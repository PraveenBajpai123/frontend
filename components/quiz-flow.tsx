...
interface QuizFlowProps {
  ...
  sessionMode: "subtopic" | "review";
  onComplete: (summary: Summary, progressPct: number) => void;
  onExit: () => void;
  onRetry?: () => void;
  onNextTopic?: () => void;
}
...
export function QuizFlow({
  ...
  const [answerHistory,  setAnswerHistory]  = useState<HistoryItem[]>([]);
  ...
  const handleNext = () => {
    ...
    if (answerResult.sessionComplete) {
      setScreen("done");
      // Fire onComplete immediately so parent can do background updates
      const pct = questionsShown > 0 ? Math.round((correct / questionsShown) * 100) : 0;
      onComplete(summary || { score: correct, total: questionsShown, grade: pct >= 60 ? "Pass" : "Needs Work", passed: pct >= 60, percentage: pct }, pct);
      return;
    }
          ...
          >
            ...
            {sessionMode === "review" ? (
              <>
                <h1
                  ...
                  style={{ fontSize: "clamp(2rem,6vw,2.5rem)", letterSpacing: "-0.03em" }}
                >
                  Memory Consolidated!
                </h1>
                <p className="text-gray-400 text-sm mb-8">
                  Your concepts have been reinforced and pushed to longer memory intervals.
                </p>
              </>
            ) : (
              <>
            ...
            {answerHistory.length > 0 && (
              <div className="mb-8 text-left space-y-4">
                <h2 className="text-white font-bold text-lg mb-2">Review Your Answers</h2>
                {answerHistory.map((item, i) => (
                  <div key={i} className="rounded-2xl p-5" style={{ background: "#1a1a1a", border: "1px solid #222" }}>
                    <h3 className="text-white font-semibold text-sm leading-snug mb-4">
                      {i + 1}. {item.question}
                    </h3>
                    <div className="space-y-2 mb-4">
                      {item.options.map((opt, j) => {
                        const isSelected = j === item.selectedOption;
                        ...
                        return (
                          ...
                          >
                            <span
                              ...
                              style={{
                                background: isCorrect ? "#4ade80" : isSelected ? "#ef4444" : "#252525",
                                color: isCorrect || isSelected ? "#141414" : "#666",
                              }}
                            >
                              {isCorrect ? "✓" : isSelected ? "✗" : String.fromCharCode(65 + j)}
                            </span>
                            <span className="text-sm mt-0.5" style={{ color: textColor }}>{opt}</span>
                          </div>
                        );
                      })}
                    </div>
                    {item.explanation && (
                      <div
                        ...
                        style={{ background: "rgba(204,235,88,0.05)", border: "1px solid rgba(204,235,88,0.15)" }}
                      >
                        <h4 className="text-xs font-bold mb-1 uppercase tracking-wider" style={{ color: "#CCEB58" }}>
                          Explanation
                        </h4>
            ...
            <div className="flex gap-3">
              {sessionMode === "review" ? (
                ...
                <>
                  ...
                  <motion.button
                    onClick={passed ? onNextTopic : onRetry}
                    whileHover={{ scale: 1.03 }}
                    whileTap={{ scale: 0.97 }}
                    className="flex-1 py-3 rounded-full font-bold text-sm"
                    style={{ background: "#CCEB58", color: "#141414" }}
